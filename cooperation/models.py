...
class CooperationType(models.Model):
    ...
    title = models.CharField(
        max_length=125, null=False, blank=False, verbose_name="نوع همکاری"
    )
...
class CooperationRequest(models.Model):
    ...
    cv = models.FileField(
        null=True, blank=True, upload_to=cv_file_path, verbose_name="رزومه"
    )
    message = models.TextField(
        blank=True, null=True, max_length=1200, verbose_name="پیام"
    )

    create_date = models.DateTimeField(auto_now_add=True, verbose_name="زمان ارسال")

    type = models.ForeignKey(
        CooperationType, on_delete=models.CASCADE, verbose_name="نوع همکاری"
    )
